    ...
    actions: {
        ...
        async editCategoryById({ commit, getters }, category) {
            ...
            try {
                ...
                // Comment next line if using FakeJSON
                // response = await axios.put('/api/categories/edit/' + id)
                let idx = getters.getCategoryIndex(category.id)
                let payload = {'idx':idx, 'category': category}
